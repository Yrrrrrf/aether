...
const DB_CONN = Deno.env.get("DB_URL") || "postgresql://postgres:postgres@127.0.0.1:54322/postgres";
const SUPABASE_URL = Deno.env.get("SUPABASE_URL") || "http://127.0.0.1:54321";
const SUPABASE_ANON_KEY = Deno.env.get("SUPABASE_ANON_KEY") || "sb_publishable_ACJWlzQHlZjBrEguHvfOxg_3BJgxAaH";
...
export async function withSupabaseEnv(
    name: string,
    fn: (ctx: TestContext) => Promise<void>,
) {
    Deno.test({
        name,
        ignore: Deno.env.get("TEST_SUPABASE") === "0",
        fn: async () => {
            // 1. Setup
